...
namespace Lombiq.Hosting.Azure.ApplicationInsights.Services
{
    ...
    {
        /// <summary>
        /// Re-logs those log entries (if not already done) that where created during application start, before the AI appender was registered.
        /// </summary>
        ...
        public void ReLogStartupLogEntriesIfNew()
        {
            ...
            if (_shellSettings.Name == ShellSettings.DefaultName && !_wasReLogged)
            {
                ...
                lock (_reLogLock)
                {
                    ...
                    var orchardAppender = hierarchyRoot.Appenders.Cast<IAppender>().FirstOrDefault(appender => appender.GetType().Name == "OrchardFileAppender");
                    if (orchardAppender != null)
                    {
                        ...
                        {
                            // Reading the log file line by line, trying to retrieve log entries created since the app start, then re-logging them.
                            // This won't cut it if the log file was changed just during startup (because e.g. midnight passed) but good enough.
                            using (var streamReader = File.OpenText(logFilePath))
                            {
                                // Checking for lines that appear to be starts of log entries, then check for those that were made after the start
                                // of the worker process (this is an approximation to get entries that were logged after the app start; actually the
                                // app can be restarted while the worker process keeps running.
                                ...
                                while ((line = streamReader.ReadLine()) != null)
                                {
                                    ...
                                    if (segments.Length > 2 &&
                                        (DateTime.TryParse(segments[0] + " " + segments[1].Split(',')[0], out logTime) || DateTime.TryParse(segments[0], out logTime)) &&
                                        logTime > startTime)
                                    {
                                        var logMessage = "Re-logging entries since app start for Application Insights. The following entries were previously logged but only to the log file, so re-logging them so they are sent to AI." +
                                            Environment.NewLine + line + Environment.NewLine + streamReader.ReadToEnd();

                                        // Re-logging with level of the logger. Re-logging is a workaround in itself where this is another one:
                                        // the re-logging message from above can even have e.g. the ERROR level, what is not accurate. Nevertheless
                                        // this is to ensure that we use the lowest possible level, trying to avoid false alarms.
